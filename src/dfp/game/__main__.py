...
import argparse
from ..deploy import parse_args, main 
...
class Game:
    def __init__(self, tomlfile: str = "docs/game.toml"):
        start = time.time()
        self.args = parse_args([])  # Pass an empty list if no command-line args are given
        self.model = Model(tomlfile=tomlfile, args=self.args)
        end = time.time()
